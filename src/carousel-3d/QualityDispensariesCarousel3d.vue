<template>
    <div class="carousel-3d-container" :style="{ height: this.slideHeight + 'px' }">
        ...
        <controls v-if="controlsVisible" :next-html="controlsNextHtml" :prev-html="controlsPrevHtml" :width="controlsWidth"
            :height="controlsHeight"></controls>
        <!-- Pagination dots -->
        <div class="pagination-dots">
            <div v-for="(slide, index) in total" :key="index" class="dot" :class="{ active: currentIndex === index }"
                @click="goSlide(index)"></div>
        </div>
    </div>
</template>
