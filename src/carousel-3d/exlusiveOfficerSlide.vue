<template>
    <div class="carousel-3d-blog-slide" :style="slideStyle" :class="computedClasses" @click="goTo()">
  ...
  export default {
    ...
    if (!this.isCurrent) {
      ...
      if (rIndex >= 0 || lIndex >= 0) {
        ...
        styles.opacity = 1; // Set opacity to 0.5 for non-current slides
        styles.visibility = "visible";
      }
  
      // Check if the neighboring slides are within the range of three active slides
      const activeSlideIndices = [this.parent.currentIndex - 1, this.parent.currentIndex, this.parent.currentIndex + 1];
      if (activeSlideIndices.includes(this.index)) {
        styles.visibility = "visible";
      }
    ...
    methods: {
      ...
      calculatePosition(i, positive, zIndex) {
        ...
        const leftRemain =
          this.parent.space === "auto" ? parseInt((i + 1) * (this.parent.width / 1.5), 10) : parseInt((i + 1) * this.parent.space, 10);
  ...
  .carousel-3d-blog-slide {
    ...
    overflow: hidden;
    top: 0;
    background: url("../assets/images/BlogCard.svg");
    border: none;
