<template>
  ...
  const sections = [
    { name: 'HomeHeroBg', class: 'HomeHeroBg', component: defineAsyncComponent(() => import('../views/PostAJobHeroSection.vue').then(m => m.default), { lazy: true }) },
    { name: '', class: '', component: defineAsyncComponent(() => import('../views/PostAJobForm.vue').then(m => m.default), { lazy: true }) },
  ];
  setTimeout(() => {
  ...
  <style scoped>
  .HomeHeroBg {
    background: url("../assets/images/bgs/PostAJobHeroBg.svg#svgView(preserveAspectRatio(none))") center center no-repeat;
